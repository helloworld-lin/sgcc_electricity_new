...
from ncc_matcher import SliderCaptchaMatcher
import platform
...
class DataFetcher:
    ...
    def __init__(self, username: str, password: str):
        ...
        # 使用NCC算法替代ONNX模型进行滑块验证码识别
        self.captcha_matcher = SliderCaptchaMatcher()
    ...
    def _get_webdriver(self):
        if platform.system() == 'Windows':
            ...
            driver = webdriver.Firefox(options=firefox_options, service=FirefoxService("/usr/local/bin/geckodriver"))
    ...
    def _login(self, driver, phone_code = False):
        ...
        if phone_code:
            ...
            for retry_times in range(1, self.RETRY_TIMES_LIMIT + 1):
                ...
                # 使用NCC算法识别滑块验证码位置
                distance = self.captcha_matcher.get_distance(background_image)
